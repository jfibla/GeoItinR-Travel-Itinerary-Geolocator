...
# Editing and geolocalization adjusted validated
...
<b>Places</b>: lugar_id | ciudad | pais | lat | lon | wikipedia
...
<br><b>Interest</b>: descriptor | lat | lon | ciudad | país | observación | tipo | enlace
<br><b>info</b>: país | documentación | hora_local | idiomas | internet | enchufes | moneda
...
<b>Places</b>: lloc_id | ciutat | país | lat | lon | wikipedia
...
<br><b>Interest</b>: descriptor | lat | lon | ciutat | país | observació | tipus | enllaç
<br><b>info</b>: país | documentació | hora_local | idiomes | internet | endolls | moneda
...
# ---- Canonical schema per sheet (internal names) ----
ORDER_CANON <- list(
  places  = c("place_id","ciudad","pais","lat","lon","wikipedia"),
  stages  = c("etapa","from_id","to_id","date","description","modo","ruta"),
  hotels  = c("place_id","hotel_name","hotel_link","details","hotel_lat","hotel_lon"),
  interes = c("descriptor","lat","lon","ciudad","pais","observacio","tipus","link")
)

# ---- Secure ONE transport column ----
ensure_one_transport <- function(df) {
  if (is.null(df) || !ncol(df)) return(df)
  nms <- names(df)
  if ("medi" %in% nms && "modo" %in% nms) {
    df$modo <- dplyr::coalesce(as.character(df$modo), as.character(df$medi))
    df$medi <- NULL
  } else if ("medi" %in% nms && !"modo" %in% nms) {
    names(df)[names(df) == "medi"] <- "modo"
  } else if (!"modo" %in% nms) {
    df$modo <- NA_character_
  }
  df
}

# ---- Select ONLY official columns and in order ----
select_and_order <- function(df, sheet) {
  cols <- ORDER_CANON[[sheet]]
  if (is.null(cols)) return(df)
  for (nm in cols) if (!nm %in% names(df)) df[[nm]] <- NA
  df[, cols, drop = FALSE]
}

# ---- Prepare table + localized headers for DT ----
prep_table <- function(td, sheet, lang_code) {
  df <- td[[sheet]]
  if (is.null(df)) df <- data.frame()
  if (identical(sheet, "stages")) df <- ensure_one_transport(df)
  df <- select_and_order(df, sheet)
  headers <- unname(HEADERS[[lang_code]][[sheet]][colnames(df)])
  list(df = df, headers = headers)
}

# ===== Column labels shown/exported by language =====----
DT_LABELS <- list(
  en = list(
    places  = c(place_id="place_id", ciudad="city", pais="country", lat="lat", lon="lon", wikipedia="wikipedia"),
    stages  = c(etapa="stage", from_id="from_id", to_id="to_id", date="date",
                description="description", modo="transport", ruta="comments"),
    hotels  = c(place_id="place_id", hotel_name="hotel_name", hotel_link="hotel_link",
                details="details", hotel_lat="hotel_lat", hotel_lon="hotel_lon"),
    interes = c(descriptor="descriptor", lat="lat", lon="lon", ciudad="city",
                pais="country", observacio="details", tipus="type", link="link"),
    info    = c(country="country", DOCUMENTATION="documentation", LOCAL_TIME="local_time",
                LANGUAGE="language", INTERNET="internet", PLUGS="plugs", CURRENCY="currency")
  ),
  es = list(
    places  = c(place_id="lugar_id", ciudad="ciudad", pais="país", lat="lat", lon="lon", wikipedia="wikipedia"),
    stages  = c(etapa="etapa", from_id="de_id", to_id="a_id", date="fecha",
                description="descripción", modo="transporte", ruta="comentarios"),
    hotels  = c(place_id="lugar_id", hotel_name="hotel_nombre", hotel_link="hotel_enlace",
                details="detalles", hotel_lat="hotel_lat", hotel_lon="hotel_lon"),
    interes = c(descriptor="descriptor", lat="lat", lon="lon", ciudad="ciudad",
                pais="pais", observacio="observación", tipus="tipo", link="enlace"),
    info    = c(country="país", DOCUMENTATION="documentación", LOCAL_TIME="hora_local",
                LANGUAGE="idiomas", INTERNET="internet", PLUGS="enchufes", CURRENCY="moneda")
  ),
  ca = list(
    places  = c(place_id="lloc_id", ciudad="ciutat", pais="país", lat="lat", lon="lon", wikipedia="wikipedia"),
    stages  = c(etapa="etapa", from_id="de_id", to_id="a_id", date="data",
                description="descripció", modo="transport", ruta="comentaris"),
    hotels  = c(place_id="lloc_id", hotel_name="hotel_nom", hotel_link="hotel_enllaç",
                details="detalls", hotel_lat="hotel_lat", hotel_lon="hotel_lon"),
    interes = c(descriptor="descriptor", lat="lat", lon="lon", ciudad="ciutat",
                pais="país", observacio="observació", tipus="tipus", link="enllaç"),
    info    = c(country="país", DOCUMENTATION="documentació", LOCAL_TIME="hora_local",
                LANGUAGE="idiomes", INTERNET="internet", PLUGS="endolls", CURRENCY="moneda")
  )
)

# Returns headers for a df based on sheet & language (only the "display name" changes)
dt_headers <- function(df, sheet, l) {
  m <- DT_LABELS[[l]][[sheet]]
  if (is.null(m)) return(names(df))
  nm <- names(df)
  out <- nm
  
  for (k in names(m)) {
    hit <- which(nm == k)
    if (length(hit)) out[hit[1]] <- m[[k]]
  }
  out
}

# Renombra columnas de salida (antes de escribir Excel) según idioma
rename_for_lang <- function(df, sheet, l) {
  m <- DT_LABELS[[l]][[sheet]]
  if (is.null(m)) return(df)
  nm <- names(df)
  for (k in names(m)) {
    hit <- which(nm == k)
    if (length(hit)) nm[hit[1]] <- m[[k]]
  }
  names(df) <- nm
  df
}
...
server <- function(input, output, session) {
  ...
  info_to_wide <- function(df) {
    ...
    df <- rename_cols_canonical(df, COL_ALIASES$info)
    
    # If tidy format: country | field | en/es/ca
    if (all(c("country","field") %in% names(df)) && any(c("en","es","ca") %in% names(df))) {
      value <- dplyr::coalesce(df$en, df$es, df$ca)
      tmp <- tibble::tibble(country = df$country, field = toupper(trimws(as.character(df$field))), value = value)
  ...
  info_to_wide_lang <- function(df, lcode = "en") {
    ...
    # ⬇️ normaliza nombres localizados a canónicos
    df <- rename_cols_canonical(df, COL_ALIASES$info)
    
    if (all(c("country","field") %in% names(df)) && any(c("en","es","ca") %in% names(df))) {
      col <- if (lcode %in% c("en","es","ca")) lcode else "en"
      tmp <- tibble::tibble(country = df$country, field = toupper(trimws(as.character(df$field))), value = as.character(df[[col]]))
      wide <- tidyr::pivot_wider(tmp, names_from = field, values_from = value)
    } else {
      wide <- df
    }
  ...
  build_trip_xlsx <- function(td, lang_code = "en") {
    ...
    # --- places (mantén internos y orden) ---
    places <- td$places
    if (!"wikipedia" %in% names(places)) places$wikipedia <- NA_character_
    places_out <- places |>
      dplyr::select(place_id, ciudad, pais, lat, lon, wikipedia)
    places_out <- rename_for_lang(places_out, "places", lang_code)
    
    # --- stages (interno: modo=ruta/transport comments) ---
    st <- td$stages
    for (nm in c("etapa","from_id","to_id","date","description","modo","ruta")) if (!nm %in% names(st)) st[[nm]] <- NA
    stages_out <- st |>
      dplyr::select(etapa, from_id, to_id, date, description, modo, ruta)
    stages_out <- rename_for_lang(stages_out, "stages", lang_code)
    ...
    if (!is.null(td$hotels) && nrow(td$hotels)) {
      ...
      hotels_out <- rename_for_lang(hotels_out, "hotels", lang_code)
    }
    ...
    if (!is.null(td$interes) && nrow(td$interes)) {
      ...
      interes_out <- inter |> dplyr::select(descriptor, lat, lon, ciudad, pais, observacio, tipus, link)
      interes_out <- rename_for_lang(interes_out, "interes", lang_code)
    }
    
    # --- info (wide) ---
    info_out <- NULL
    if (!is.null(td$info) && nrow(td$info)) {
      info_out <- info_to_wide_lang(td$info, "en")     # estructura completa
      info_out <- rename_for_lang(info_out, "info", lang_code)
    }
  ...
  sanitize_filename <- function(x) {
    x <- trimws(as.character(x %||% "itinerary_view"))
    x <- gsub("[^[:alnum:]_\\x2D ]+", "_", x)  # solo letras, números, guion y espacio
    if (!nzchar(x)) x <- "itinerary_view"
    x
  }
  
  output$download_itinerary_proc <- downloadHandler(
    filename = function(){
      paste0(sanitize_filename(input$www_filename_proc), ".xlsx")
    },
    content  = function(file){
      td <- current_td(); req(td)
      payload <- build_trip_xlsx(td, lang_code = lang())
      writexl::write_xlsx(payload, path = file)
    }
  )
  
  observeEvent(input$download_itinerary_proc, {
    req(nzchar(sanitize_filename(input$www_filename_proc)))
  })
  ...
  COL_ALIASES <- list(
    ...
    info = list(
      country       = c("country","pais","país","païs"),
      ...
      language      = c("language","idioma","idiomas","llengua","idiomes"),
      internet      = c("internet"),
      plugs         = c("plugs","enchufes","endolls","conectores","conectors"),
  ...
  read_trip_data <- function(path) {
    ...
    if (!is.na(sh_info)) {
      ...
      raw <- rename_cols_canonical(raw, COL_ALIASES$info)
      
      has_wide <- all(c("country","documentation","local_time","language","internet","plugs","currency") %in% names(raw))
 ...
 ######################################
  output$sidebar_text_blocks <- renderUI({
    ...
    # ⚠️ snapshot, para que escribir no reactive el renderUI
    default_name <- isolate(input$www_filename_proc)
    if (is.null(default_name) || !nzchar(default_name)) default_name <- "itinerary_view"
    
    label_text <- if (allow) tr("return_to_visor", l) else tr("allow_edit", l)
    base_toggle <- checkboxInput("allow_edit_process", label_text, value = allow)
    ...
    if (allow) {
      tagList(
        ...
        textInput("www_filename_proc", tr("file_name", l), value = default_name),
  ...
  ############################################################################## 
  # --- Esquema canónico fijo por tabla (orden estable) ---
  SCHEMA <- list(
    places  = c("place_id","ciudad","pais","lat","lon","wikipedia"),
    stages  = c("etapa","from_id","to_id","date","description","modo","ruta"),
    hotels  = c("place_id","hotel_name","hotel_link","details","hotel_lat","hotel_lon"),
  ...
  # --- Encabezados visibles por idioma ---
  HEADERS <- list(
    en = list(
      places  = c("place_id","city","country","lat","lon","wikipedia"),
      stages  = c("stage","from_id","to_id","date","description","transport","comments"),
      hotels  = c("place_id","hotel_name","hotel_link","details","hotel_lat","hotel_lon"),
      interes = c("descriptor","lat","lon","city","country","details","type","link")
    ),
    es = list(
      places  = c("place_id","ciudad","pais","lat","lon","wikipedia"),
      stages  = c("etapa","de_id","a_id","fecha","descripción","transporte","comentarios"),
      hotels  = c("lugar_id","hotel_nombre","hotel_enlace","detalles","hotel_lat","hotel_lon"),
      interes = c("descriptor","lat","lon","ciudad","pais","observación","tipo","enlace")
    ),
    ca = list(
      places  = c("place_id","ciutat","país","lat","lon","wikipedia"),
      stages  = c("etapa","de_id","a_id","data","descripció","transport","comentaris"),
      hotels  = c("lloc_id","hotel_nom","hotel_enllaç","detalls","hotel_lat","hotel_lon"),
      interes = c("descriptor","lat","lon","ciutat","país","observació","tipus","enllaç")
    )
  )
  
  ############################################################################## 
  prep_table <- function(td, kind, lcode) {
    df <- td[[kind]]
    if (is.null(df)) df <- data.frame()
    
    canon <- SCHEMA[[kind]]
    for (nm in canon) if (!nm %in% names(df)) df[[nm]] <- NA
    df <- df[, canon, drop = FALSE]
    
    for (nm in intersect(names(df), c("lat","lon","hotel_lat","hotel_lon"))) {
      df[[nm]] <- suppressWarnings(as.numeric(df[[nm]]))
    }
    
    headers <- HEADERS[[lcode]][[kind]]  # etiquetas visibles, NO se asignan a names(df)
    list(df = df, headers = headers)
  }
  ############################################################################## 
  # columns with URLs in the CANONICAL schema (language independent)
  link_cols_canonical <- function(slot) {
    switch(tolower(slot),
           "places"  = c("wikipedia"),
           "hotels"  = c("hotel_link"),
           "interes" = c("link"),
  ...
  # 'escape' by indexes on the CANONICAL names of the df we show
  dt_escape_for <- function(df, slot) {
    nm <- colnames(df)
    no_escape <- intersect(nm, link_cols_canonical(slot))
    esc_idx <- setdiff(seq_along(nm), match(no_escape, nm, nomatch = 0))
    if (length(esc_idx)) esc_idx else TRUE
  }
  

  ############################################################################## 
  # ---- Edit wrapper with localized columns (index -> canonical name) ----
  register_edit_localized <- function(input_id, slot_name, table_proxy_id) {
    observeEvent(input[[paste0(input_id, "_cell_edit")]], {
      info <- input[[paste0(input_id, "_cell_edit")]]
      td   <- trip_data(); req(td)
      
      # Table shown (always with CANONICAL names in colnames)----
      pt <- prep_table(td, slot_name, lang())
      shown_df <- pt$df
      if (is.null(shown_df) || !ncol(shown_df)) return()
      
      # VERY IMPORTANT: DataTables sends the column in base 0 -> add +1
      j <- as.integer(info$col) + 1L
      i <- as.integer(info$row)
      
      # save
      if (is.na(i) || is.na(j)) return()
      if (i < 1 || i > nrow(shown_df)) return()
      if (j < 1 || j > ncol(shown_df)) return()
      
      # Canonical column to update
      col_canon <- colnames(shown_df)[j]
      if (!col_canon %in% names(td[[slot_name]])) return()
      
      # Write value with correct type
      td[[slot_name]][i, col_canon] <- coerce_like(info$value, td[[slot_name]][[col_canon]])
      
      # Save and refresh
      trip_data(td)
      DT::replaceData(DT::dataTableProxy(table_proxy_id),
                      prep_table(td, slot_name, lang())$df,
                      resetPaging = FALSE, rownames = FALSE)
    })
  }
  
  # Records
  register_edit_localized("places_table",  "places",  "places_table")
  register_edit_localized("stages_table",  "stages",  "stages_table")
  register_edit_localized("hotels_table",  "hotels",  "hotels_table")
  register_edit_localized("interes_table", "interes", "interes_table")
 ...
 output$info_table <- DT::renderDT({
   ...
   DT::datatable(
     wide,
     colnames = dt_headers(wide, "info", lang()),
...
#
  ...
  output$places_table <- DT::renderDT({
    td <- current_td(); req(td)
    pt <- prep_table(td, "places", lang())
    DT::datatable(
      pt$df,
      colnames = pt$headers,             # ⬅️ AÑADIR
      rownames  = FALSE,
      selection = if (isTRUE(input$allow_edit_process)) "multiple" else "none",
      editable  = isTRUE(input$allow_edit_process),
      escape    = dt_escape_for(pt$df, "places"),   # ⬅️ ver punto 3
      options   = list(pageLength = 8)
    )
  })
  
  output$stages_table <- DT::renderDT({
    td <- current_td(); req(td)
    pt <- prep_table(td, "stages", lang())
    DT::datatable(
      pt$df,
      colnames = pt$headers,             # ⬅️
      rownames  = FALSE,
      selection = if (isTRUE(input$allow_edit_process)) "multiple" else "none",
      editable  = isTRUE(input$allow_edit_process),
      escape    = dt_escape_for(pt$df, "stages"),   # ⬅️
      options   = list(pageLength = 8)
    )
  })
  
  output$hotels_table <- DT::renderDT({
    td <- current_td(); req(td)
    pt <- prep_table(td, "hotels", lang())
    DT::datatable(
      pt$df,
      colnames = pt$headers,             # ⬅️
      rownames  = FALSE,
      selection = if (isTRUE(input$allow_edit_process)) "multiple" else "none",
      editable  = isTRUE(input$allow_edit_process),
      escape    = dt_escape_for(pt$df, "hotels"),   # ⬅️
      options   = list(pageLength = 8)
    )
  })
  
  output$interes_table <- DT::renderDT({
    td <- current_td(); req(td)
    pt <- prep_table(td, "interes", lang())
    DT::datatable(
      pt$df,
      colnames = pt$headers,             # ⬅️
      rownames  = FALSE,
      selection = if (isTRUE(input$allow_edit_process)) "multiple" else "none",
      editable  = isTRUE(input$allow_edit_process),
      escape    = dt_escape_for(pt$df, "interes"),  # ⬅️
      options   = list(pageLength = 8)
    )
  })
  ...
  observeEvent(input$info_table_cell_edit, {
    ...
    # If they try to edit 'country' (col 1), we ignore
  ...
  observeEvent(input$auto_complete, {
    ...
    isolate({
      td2 <- get_td()
      DT::replaceData(DT::dataTableProxy("places_table"),
                      prep_table(td2, "places",  lang())$df,
                      resetPaging = FALSE, rownames = FALSE)
      DT::replaceData(DT::dataTableProxy("hotels_table"),
                      prep_table(td2, "hotels",  lang())$df,
                      resetPaging = FALSE, rownames = FALSE)
      DT::replaceData(DT::dataTableProxy("interes_table"),
                      prep_table(td2, "interes", lang())$df,
                      resetPaging = FALSE, rownames = FALSE)
    })
